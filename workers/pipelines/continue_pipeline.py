...
def run_continue(
    ...
    async def _create_runs():
        async with async_session() as db:
            continue_run = await db.get(GenerationRun, run_id)
            if continue_run:
                continue_run.status = "running"
                await db.commit()
    ...
    try:
        ...
        async def _mark_failed():
            async with async_session() as db:
                continue_run = await db.get(GenerationRun, run_id)
                if continue_run:
                    continue_run.status = "failed"
                    continue_run.error = str(exc)
                    await db.commit()
